...
static void	put_player(t_data *data, int x, int y)
{
	data->x_player = x;
	data->y_player = y;
	mlx_put_image_to_window(data->con_id, data->window,
		data->player, x * 50, y * 50);
}

static void	put_img(t_data *data, void *img, int x, int y)
{
	mlx_put_image_to_window(data->con_id,
		data->window, img, x * 50, y * 50);
}
...
static void	put_exit(t_data *data, int x, int y)
{
	if (data->n_collect == 0)
	{
		...
		data->exit = mlx_xpm_file_to_image(data->con_id, DEX,
